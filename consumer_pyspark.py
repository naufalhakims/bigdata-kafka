...
# Filter suhu tinggi dan kelembaban tinggi
...
peringatan_suhu = filtered_suhu.selectExpr(
    "'[ALERT] Suhu Melebihi Batas Aman' as warning_type",
    "gudang_id",
    "suhu",
    "timestamp"
)
...
peringatan_kelembaban = filtered_kelembaban.selectExpr(
    "'[ALERT] Kelembaban Terlalu Tinggi' as warning_type",
    "gudang_id",
    "kelembaban",
    "timestamp"
)
...
joined_status = joined_stream.withColumn(
    ...
    expr("""
        CASE
            WHEN suhu > 80 AND kelembaban > 70 THEN 'Kondisi kritis! Barang berpotensi rusak.'
            WHEN suhu > 80 THEN 'Suhu tinggi terdeteksi, kelembaban masih aman.'
            WHEN kelembaban > 70 THEN 'Kelembaban tinggi, suhu dalam batas normal.'
            ELSE 'Situasi terkendali'
        END
    """)
)
